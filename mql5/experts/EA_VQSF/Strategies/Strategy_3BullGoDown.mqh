#ifndef __STRATEGY_3BULLGODOWN_MQH__
#define __STRATEGY_3BULLGODOWN_MQH__
...
class CStrategy_3BullGoDown : public CBaseStrategy
{
private:
   ...
   void Reset()
   {
      ...
      if(logger) logger.LogEvent(GetName(), "RESET", "Estado resetado.");
   }
...
public:
   ...
   string GetName() override { return "3BullGoDown"; }
   ...
   TradeSignal OnUpdate(CLogger &log) override
   {
      ...
      if(isBull)
      {
         if(bodySize >= config.MinCandleSize && bodySize <= config.MaxCandleSize)
         {
            ...
            if(state.sequence >= config.MinSequence)
            {
               ...
               {
                  log.LogDebug(m_name, "WARN", "Lower wick too large: " + DoubleToString(lowerWick, 2) + " > " + DoubleToString(2 * bodySize, 2) + " - rejecting signal");               
                  return signal;
               }
              ...
              // int validationResult = ValidateCloseAgainstHistory(-1, config.Timeframe, 20);
               ...
               {
                  log.LogDebug(m_name, "WARN", "Close equal to 20-bar max - no signal generated");
                  return signal;
               }
      ...
      {       
         log.LogDebug(m_name, "INFO", "Bear candle detected - resetting sequence from " + IntegerToString(state.sequence));
         Reset();
         return signal;
      }
